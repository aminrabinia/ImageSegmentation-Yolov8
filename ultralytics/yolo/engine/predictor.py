...
class BasePredictor:
    ...
    def __call__(self, source=None, model=None, verbose=False, stream=False):
        if stream:
            ...
            _ = list(chain(*list(self.stream_inference(source, model, verbose))))  # merge list of Result into one
    ...
    def stream_inference(self, source=None, model=None, verbose=False):
        ...
        for batch in self.dataset:
            ...
            for i in range(len(im)):
                ...
                if self.args.show:
                    self.annotator = self.write_results(i, results, (p, im, im0))
    ...
    def setup_model(self, model):
        ...
        self.model = AutoBackend(model, device=device, dnn=self.args.dnn, fp16=self.args.half, fuse=False)
        self.device = device
        self.model.eval()
